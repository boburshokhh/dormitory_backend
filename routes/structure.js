  ...
  try {
    ...
    for (const floor of floorsResult.rows) {
      ...
      if (dormitory.type === 'type_1') {
        ...
        floorData.rooms = await Promise.all(roomsResult.rows.map(async (room) => {
          const bedsResult = await query(
            'SELECT id, bed_number, is_occupied, student_id FROM beds WHERE room_id = $1 AND is_active = true ORDER BY bed_number',
            [room.id]
          );
        ...
        floorData.blocks = await Promise.all(blocksResult.rows.map(async (block) => {
          const roomsResult = await query(`
            SELECT r.*,
              COUNT(b.id) as bed_count,
              COUNT(CASE WHEN b.is_occupied = true THEN 1 END) as occupied_beds
...
router.post('/dormitories/:dormitoryId/floors', 
  requireAdmin, 
...
router.post('/floors/:floorId/rooms',
  requireAdmin,
  ...
  async (req, res) => {
    try {
      ...
      const result = await transaction(async (client) => {
        ...
        // Создаем 2 места
        await client.query(
          'INSERT INTO beds (room_id, bed_number) VALUES ($1, 1), ($1, 2)',
          [room.id]
        );
...
router.post('/floors/:floorId/blocks',
  requireAdmin,
  validateUUID('floorId'),
